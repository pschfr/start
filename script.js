...
function getWeather(location) { // Gets weather for requested location, appends to page
	$.simpleWeather({
		location: location,
...
function refreshStuffs() { // Master refresh function; appends random greeting, quote, and background
...
function bindMousetraps() { // Initializes main keyboard nav
...
Mousetrap.bind('esc', function(e) { // Esc to close all tabs, leave secret mode
	var randNum = Math.floor((Math.random() * TOTAL_PRESETS));
	$('body').removeClass('rnbw').addClass('bg' + (randNum + 1));
	$('.subMenu').slideUp(150);
	$('li a').removeClass('active');
	console.log('you escaped!');
	return false;
});
Mousetrap.bind('space', function(e){ // Refreshes everything, and closes all cells
	$('.subMenu').slideUp(150);
	$('li a').removeClass('active');
	refreshStuffs();
	return false;
});
Mousetrap.bind('up up down down left right left right b a enter', function() { // SECRET PARTY MODE!!1! :D
	$('body').attr('class', function(i, c){
    	return c.replace(/(^|\s)bg\S+/g, '');
	}).addClass('rnbw');
});
$(function() { // Does everything on page load, sets it to auto-refresh every 30s
	$("ul.subMenu").hide();
